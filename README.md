...
Run [`supabase/schema.sql`](supabase/schema.sql) in Supabase SQL Editor, then add the values from [`.env.example`](.env.example). Add `OPENAI_API_KEY` for live AI messages and Google OAuth credentials for Gmail sending and Calendar event creation. Without Supabase variables, the UI explicitly runs in demo mode.
